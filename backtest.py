...
ASSETS = [
    ...
    {"name": "Kupfer",         "id": "HG=F"},
    {"name": "Weizen",         "id": "ZW=F"},
    {"name": "Short S&P 500",  "id": "XSPS.L", "short": True},
    {"name": "Short DAX",      "id": "DXSN.DE", "short": True},
    {"name": "Short Nasdaq",   "id": "QQQS.L", "short": True},
    {"name": "Short Krypto",   "id": "BITI", "short": True},
]
...
# ── Backtest-Kern ─────────────────────────────────────
def backtest_params(asset_preise_list, params):
    kauf = params["kauf"]
    verk = params["verk"]
    sl_m = params["sl"]
    tp_m = params["tp"]
    ...
    for preise, is_short in asset_preise_list:
...
# ── Datenfehler-Check ─────────────────────────────────
def pruefe_datenfehler(assets_mit_preisen):
    """Prüft ob verschiedene Assets identische Kursdaten bekommen haben."""
    warnungen = []
    preis_fingerprints = {}

    for name, preise in assets_mit_preisen:
        if preise is None or len(preise) < 5:
            continue
        fp = tuple(round(p, 4) for p in preise[-5:])
...
def main():
    ...
    for asset in ASSETS:
        ...
        if p:
            ...
            asset_preise_list.append((p, is_short))
            assets_fuer_check.append((asset["name"], p))
    print(f"{len(asset_preise_list)} Assets geladen")

    # ── FIX: Datenfehler-Check ─────────────────────────
    datenfehler = pruefe_datenfehler(assets_fuer_check)
    if datenfehler:
        print("\n".join(datenfehler))

    msg = f"<b>Backtest Optimierung – {heute}</b>\n"
    msg += f"📊 {PERIODE} Zeitraum – {len(asset_preise_list)} Assets\n"
    if datenfehler:
        msg += f"⚠️ {len(datenfehler)} Datenfehler erkannt!\n"
    msg += "\n"
    ...
    for params in PARAMETER_SETS:
        ...
        r = backtest_params(asset_preise_list, params)
        diff = r["bot"] - r["hold"]
    ...
    export = {
        "datum": heute,
        "periode": PERIODE,
        "assets_count": len(asset_preise_list),
        "datenfehler": len(datenfehler),
