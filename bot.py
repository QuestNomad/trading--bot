...
def ema(p, n): return pd.Series(p).ewm(span=n, adjust=False).mean()
...
def macd_val(p):
    ...
    return m.iloc[-1], m.ewm(span=9).mean().iloc[-1]

def atr_val(p, n=14):
    s = pd.Series(p)
    return (s.rolling(2).max()-s.rolling(2).min()).rolling(n).mean().iloc[-1]

def berechne_signal(preise, sw, seu):
    if len(preise) < 200: return "⏳ WARTEN", 0, {}
    aktuell = preise[-1]
    s200 = sma(preise, 200).iloc[-1]
    s50 = sma(preise, 50).iloc[-1]
    r = rsi_val(preise)
    ...
    if aktuell < (sma(preise,20).iloc[-1] + 2*pd.Series(preise).rolling(20).std().iloc[-1]): punkte += 1
    sl = aktuell-(a*2)
    tp = aktuell+(a*4)
...
def run_bot():
    ...
    for e in top:
        ...
        schreibe_journal(asset["name"], e["signal"], aktuell, details, sw, seu)
