...
def schreibe_journal(asset_name, signal, kurs, details, sw, seu):
    try:
        ...
        with open(journal_file, "a", newline="", encoding="utf-8") as f:
            ...
            if not file_exists:
                writer.writerow([
                    "Datum", "Asset", "Signal", "Kurs", "SMA200", "RSI",
                    "Score", "Stop Loss", "Take Profit",
                    "Sentiment Welt", "Sentiment EU", "Kommentar"
                ])
            writer.writerow([
                datetime.now().strftime("%d.%m.%Y %H:%M"),
                asset_name, signal, round(kurs, 2),
    ...
    try:
        ...
        if sheets_url:
            payload = {
                ...
                "sentiment_welt": sw, "sentiment_eu": seu,
                "ergebnis": "", "kommentar": "Paper Trading"
            }
...
def erstelle_chart(preise, daten, name, signal, details):
    ...
    ax1.plot(daten[-100:], preise[-100:], color='#89b4fa',
             linewidth=2, label='Kurs')
    ...
    ax1.axhline(y=details["stop_loss"], color='#f38ba8',
                linestyle=':', linewidth=1.5,
                label=f'SL: {details["stop_loss"]:.0f}')
    ax1.axhline(y=details["take_profit"], color='#a6e3a1',
                linestyle=':', linewidth=1.5,
                label=f'TP: {details["take_profit"]:.0f}')
...
def pruefe_datenfehler(ergebnisse):
    ...
    for e in ergebnisse:
        if e is None or e["preise"] is None or len(e["preise"]) < 5:
            continue
        # Fingerprint aus den letzten 5 Kursen
